    ...
    for( blk = 1; blk <= A.nblocks; blk++ )
    {
        switch( A.blocks[blk].blockcategory )
        {
            case DIAG:
                ...
                break;
            case MATRIX:
                temp = norm2( A.blocks[blk].blocksize * A.blocks[blk].blocksize, A.blocks[blk].data.mat );
                nrm  += temp * temp;
                break;
            case PACKEDMATRIX:
            default:
    ...
    for( blk = 1; blk <= A.nblocks; blk++ )
    {
        switch( A.blocks[blk].blockcategory )
        {
            case DIAG:
                ...
                break;
            case MATRIX:
                temp = norm1( 1 * A.blocks[blk].blocksize * A.blocks[blk].blocksize, A.blocks[blk].data.mat );
                nrm  += temp;
                break;
            case PACKEDMATRIX:
            default:
