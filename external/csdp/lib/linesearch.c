...
double linesearch( n, dX, work1, work2, work3, cholinv, q, z, workvec, stepfrac, start, printlevel )
    int                n;
    ...
    int                printlevel;
{
    ...
    {
        if( (work1.blocks[blk].blocksize > maxn) && (work1.blocks[blk].blockcategory == MATRIX) )
    ...
    for( j = 1; j <= LANCZOSITS; j++ )
    {
        ...
        if( (j >= 7) && (maxeigs[j] <= 1 / (3 * start)) && (fabs( (maxeigs[j] - maxeigs[j - 2]) / (0.000001 + fabs( maxeigs[j] )) ) < 0.2) )
        {
            if( printlevel >= 4 )
