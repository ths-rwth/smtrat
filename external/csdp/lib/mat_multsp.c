            ...
            switch( A.blocks[blk].blockcategory )
            {
                case DIAG:
                    for( i = 1; i <= A.blocks[blk].blocksize; i++ )
                        C.blocks[blk].data.vec[i] = scale1 * A.blocks[blk].data.vec[i] * B.blocks[blk].data.vec[i];
                    break;
                case MATRIX:
                    ...
                    if( ptr->numentries / (blksize * blksize * 1.0) > SPARSELIMB )
                    {
                         ...
                         */
                        mat_mult_raw( blksize, scale1, scale2, A.blocks[blk].data.mat, B.blocks[blk].data.mat, C.blocks[blk].data.mat );
                    }
                    else
                    {
                        #ifdef __GNUC__
...

                        {
                            ...
                            for( ii = 1; ii <= ptr->numentries; ii++ )
                            {
                                ...
                                if( (q % total_threads) == thread_num )
                                {
                                    ...
                                    for( i = 1; i <= blksize; i++ )
                                        C.blocks[blk].data.mat[ijtok( i, q, blksize )] += temp * A.blocks[blk].data.mat[ijtok( i, p, blksize )];
                                }
                                ;
            ...
            switch( C.blocks[blk].blockcategory )
            {
                ...
                case MATRIX:
                    ...
                    for( j = 1; j <= C.blocks[blk].blocksize; j++ )
                        for( i = 1; i <= C.blocks[blk].blocksize; i++ )
                            C.blocks[blk].data.
                                mat[ijtok( i, j, C.blocks[blk].blocksize )] = scale2 * C.blocks[blk].data.mat[ijtok( i, j, C.blocks[blk].blocksize )];
                    break;
                case PACKEDMATRIX:
                default:
                    printf( "mat_multsp illegal block type \n" );
            ...
            switch( A.blocks[blk].blockcategory )
            {
                ...
                case MATRIX:
                    ...
                    if( ptr->numentries / (blksize * blksize * 1.0) > SPARSELIMB )
                    {
                         ...
                         * A dense block.  Do it the old fashioned way.
                         */
                        mat_mult_raw( blksize, scale1, 1.0, A.blocks[blk].data.mat, B.blocks[blk].data.mat, C.blocks[blk].data.mat );
                    }
                    else
                    {
                        #ifdef __GNUC__
                            ...
                            for( ii = 1; ii <= ptr->numentries; ii++ )
                            {
                                ...
                                if( (p % total_threads) == thread_num )
                                {
                                    ...
                                    for( i = 1; i <= ptr->blocksize; i++ )
                                        C.blocks[blk].data.mat[ijtok( p, i, blksize )] += temp * B.blocks[blk].data.mat[ijtok( i, q, blksize )];
                                }
                                ;
                ...
                case MATRIX:
                    ...
                    else
                    {
                        ...
                        for( ii = 1; ii <= ptr->numentries; ii++ )
                        {
                            ...
                            temp = 0;
                            for( i = 1; i <= ptr->blocksize; i++ )
                                temp += A.blocks[blk].data.mat[ijtok( i, p, blksize )] * B.blocks[blk].data.mat[ijtok( i, q, blksize )];
