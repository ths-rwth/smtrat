    ...
    struct blockmatrix work2;
{
    ...
    {
        if( X.blocks[blocknum].blockcategory != DIAG && X.blocks[blocknum].blocksize > max_blksize )
        {
            max_blknum  = blocknum;
    ...
    if( max_blknum > 0 )
    {
        work = (double**)malloc( sizeof(double*) * (max_threads * 2 + 1) );
        if( work == NULL )
        {
        ...
        {
            work[i * 2 + 1] = (double*)malloc( sizeof(double) * X.blocks[max_blknum].blocksize * X.blocks[max_blknum].blocksize );
            work[i * 2 + 2] = (double*)malloc( sizeof(double) * X.blocks[max_blknum].blocksize * X.blocks[max_blknum].blocksize );
    ...
    for( i = 1; i <= k; i++ )
    {
        ...
        while( ptri != NULL )
        {
                    ...
                    while( (p <= ptri->numentries) && (q <= ptrj->numentries) )
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                 ...
                                 * Match!
                                 */
                                contrib += ptri->entries[p] * ptrj->entries[q] * Ziblk[ptri->iindices[p]] * Xblk[ptri->iindices[p]];
                                p       = p + 1;
                                q       = q + 1;
                            }
                            ;
            ...
            /* Sparse matrix block */
            else if( ptri->issparse == 1 && X.blocks[blocknum].blockcategory == MATRIX )
            {
                        ...
                        for( ii = 1; ii <= ptri->numentries; ii++ )
                        {
                            ...
                            if( p == q )
                            {
                                for( jj = 1; jj <= ptrj->numentries; jj++ )
                                {
                                    ...
                                    {
                                        /* here p==q, r==s */
                                        contrib += enti * entj * Ziblk[ijtok( r, q, blocksize )] * Xblk[ijtok( s, p, blocksize )];
                                    }
                                    else
                                    {
                                        /* here p=q, r!=s */
                                        contrib += enti * entj
                                                   * (Ziblk[ijtok( r, q, blocksize )] * Xblk[ijtok( s, p, blocksize )]
                                                      + Ziblk[ijtok( s, q, blocksize )] * Xblk[ijtok( r, p, blocksize )]);
                                    }
                                }
                            }
                            else
                            {    /* p!= q */
                                for( jj = 1; jj <= ptrj->numentries; jj++ )
                                {
                                    ...
                                    if( r == s )
                                    {
                                        ...
                                        contrib += enti * entj
                                                   * (Ziblk[ijtok( r, q, blocksize )] * Xblk[ijtok( s, p, blocksize )]
                                                      + Ziblk[ijtok( r, p, blocksize )] * Xblk[ijtok( s, q, blocksize )]);
                                    }
                                    else
                                    {
                                        /* here, p!=q and r!=s */
                                        contrib += enti * entj
                                                   * (Ziblk[ijtok( r, q, blocksize )] * Xblk[ijtok( s, p, blocksize )]
                                                      + Ziblk[ijtok( r, p, blocksize )] * Xblk[ijtok( s, q, blocksize )]
                                                      + Ziblk[ijtok( s, q, blocksize )] * Xblk[ijtok( r, p, blocksize )]
                                                      + Ziblk[ijtok( s, p, blocksize )] * Xblk[ijtok( r, q, blocksize )]);
                                    }
                                }
                            }
                        }
            ...
            else
            {
                ...
                while( ptrj != NULL )
                {
                     ...
                     * equal to j
                     */
                    if( ptrj->issparse == 1 || ptri->numentries > ptrj->numentries || (ptri->numentries == ptrj->numentries && i <= j) )
                    {
                        contrib = 0.0;
