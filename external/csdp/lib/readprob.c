...

{
    ...
    *pn = 0;
    ...
    do
    {
        ...
        if( (matno < 0) || (matno > *pk) || (blkno < 1) || (blkno > nblocks) || (indexi < 1) || (indexi > pC->blocks[blkno].blocksize)
    ...
    for( i = 1; i <= nblocks; i++ )
    {
        if( (pC->blocks[i].blockcategory != DIAG) && (isdiag[i] == 1) && (pC->blocks[i].blocksize > 1) )
        {
            /*
