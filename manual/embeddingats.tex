...
\begin{itemize}
	\item \begin{verbatim}bool inform( const Formula* const )\end{verbatim}
	...
	\item \begin{verbatim}bool add( const Formula* )\end{verbatim}
        ...
        satisfiability check. The method returns \false, if it is easy to decide whether the given formula is not satisfiable.
	\item \begin{verbatim}ModuleInput::iterator remove( ModuleInput::iterator )\end{verbatim}
    ...
    \item \begin{verbatim}Answer check()\end{verbatim}
    	...
    	\False, if the formulas are not satisfiability, and \Unknown, if the composition
