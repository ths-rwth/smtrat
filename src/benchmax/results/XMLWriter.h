...
public:
	...
	void write(const std::map<const Tool*, std::size_t>& tools, const Results& results) {
		...
		for (const auto& tool: tools) toolNames.insert(sanitize(tool.first->binary().filename().native(), true));
		...
		for (const auto& res: results) {
			mFile << "\t<benchmarkset name=\"" << sanitize(removePrefix(res.first.native(), Settings::pathPrefix), true) << "\">" << std::endl;
			for (const auto& file: res.second.files) {
				...
				for (const auto& tool: tools) {
					...
					mFile << "\t\t\t<run solver_id=\"" << sanitize(tool.first->binary().filename().native(), true) << "\" timeout=\"" << seconds(Settings::timeLimit).count() << "s\">" << std::endl;
