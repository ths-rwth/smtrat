...
namespace cad {
	...
	private:
		...
		std::vector<std::vector<std::pair<UPoly,Origin>>> mPolynomials;
		
		void addToProjection(std::size_t level, const UPoly& p, const Origin& origin) {
	...
	public:
		...
		void addPolynomial(const UPoly& p, std::size_t cid) {
			...
			addToProjection(0, p, Origin(cid));
		}
		void removePolynomial(const UPoly& p, std::size_t cid, const std::function<void(std::size_t,SampleLiftedWith)>& callback) {
			for (std::size_t lvl = 0; lvl < dim(); lvl++) {
				//while (mPolynomials[lvl].back().second == cid) {
