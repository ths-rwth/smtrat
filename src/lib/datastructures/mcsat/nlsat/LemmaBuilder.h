...
enum class LemmaStrategy { ORIGINAL, NEW };

namespace LemmaHelper {
...
class LemmaBuilder {
private:
	const LemmaHelper::Assignment& mAssignment;
	...
	void generate_new_assignmentPropagation(const FormulasT& borders, const Callback& cb) {
		FormulasT base;
		LemmaHelper::collectAssignments(mAssignment, base, borders, true);
		...
		for (const auto& b: borders) {
			...
			cb(LemmaHelper::buildImplication(std::move(tmpo), b));
			...
			cb(LemmaHelper::buildImplication(std::move(tmpb), bneg));
			//cb(LemmaHelper::buildImplication({b}, bneg));
			//cb(LemmaHelper::buildImplication({bneg}, b));
		}
	}
	template<typename Callback>
	void generate_new_mainPropagation(const Callback& cb) {
		cb(LemmaHelper::buildImplication(carl::vector_concat(mExplanation), mConclusion));
	}
	template<typename Callback>
	void generate_new(const Callback& cb) {
		...
		FormulasT base;
		LemmaHelper::collectAssignments(mAssignment, base, mExplanation);
		cb(LemmaHelper::buildImplication(std::move(base), mConclusion));
	}
	...
	void generate_original(const Callback& cb) {
		...
		FormulasT base;
		//LemmaHelper::collectAssignments(mAssignment, base, mExplanation, true);
		//LemmaHelper::collectAssignments(mAssignment, base, { mConclusion }, true);
		...
		cb(LemmaHelper::buildImplication(std::move(base), mConclusion, true));
	}
	
public:
	LemmaBuilder(const LemmaHelper::Assignment& assignment, const FormulaT& F, const ExplanationGenerator& eg): mAssignment(assignment), mConclusion(F) {
		eg.generateExplanation(mConclusion, mExplanation);
	}

	template<typename Callback>
	void generateLemmas(const Callback& cb, LemmaStrategy strategy) {
		switch (strategy) {
			case LemmaStrategy::NEW:
				generate_new(cb);
				break;
			case LemmaStrategy::ORIGINAL:
