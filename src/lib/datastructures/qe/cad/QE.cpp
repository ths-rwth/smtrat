...
using smtrat::cad::Assignment;
  ...
  QE::QE(const FormulaT& quantifierFreePart, const QEQuery& quantifiers) {
  ...
  FormulaT QE::eliminateQuantifiers() {
    constructCAD();
  ...
  void QE::updateCAD(std::vector<Poly>& polynomials) {
  ...
  bool QE::truthBoundaryTest(TreeIT& b, TreeIT& c, TreeIT& d) {
    std::vector<TreeIT> children_b;
    std::vector<TreeIT> children_c;
    std::vector<TreeIT> children_d;
  ...
  FormulaT QE::constructImplicant(const TreeIT& sample) {
    Assignment assignment;
    Model model;
  ...
  FormulaT QE::constructSolutionFormula() {
