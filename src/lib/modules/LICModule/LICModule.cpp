...
namespace smtrat
{
    ...
    Answer LICModule<Settings>::checkCore( bool _full )
    {
        ...
        Answer res = runBackends(_full);
        
        return res; // This should be adapted according to your implementation.
    }
    ...
    Answer LICModule<Settings>::analyzeCycle(const Graph& g, const Vertex& root) {
        ...
        if (carl::isZero(sum.r)) {
            if (sum.strict) {
                ...
                for (const auto& v: others) mAssignments[g[v].var] = 0;
