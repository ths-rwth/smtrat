...
namespace smtrat
{
    ...
    bool LRAModule::inform( const Constraint* const _constraint )
    {
		...
		for( cons_bool_pair_vec::const_iterator cbPair = iter->second.begin();
			 cbPair != iter->second.end();
			 ++cbPair )
		{
...
//						return mLRASolverConstraints.end();
			...
			return mLRASolverConstraints.insert
			( 
				pair< const Constraint* const, cons_bool_pair_vec >
				(
					_constraint			   , 
					constraintPolarityPairs 
				)
			).first;
		}
    }
