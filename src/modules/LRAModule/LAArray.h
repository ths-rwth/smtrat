...
template <class T>
class LAArray : public std::vector< T >
{
protected:
  unsigned len;          // tracks the quantity of non empty elements
  std::vector<bool> is_there; // used to perform the fast check if the value is in the row
  std::vector<int> pool;      // keeps the pool of indexes of empty elements in array
...
public:
  ...
  inline int free_pos( )
  {
    ...
    {
      assert( pool.back( ) >= 0 );
      assert( pool.back( ) < static_cast<int> ( std::vector<T>::size( ) ) );
      return pool.back( );
    }
  }
  ...
  {
    typename LAArray<T>::iterator it = std::vector<T>::begin( );
  ...
  {
    assert( it - std::vector<T>::begin( ) >= 0 );
    assert( it - std::vector<T>::begin( ) < static_cast<int>( std::vector<T>::size( ) ));
    return ( it - std::vector<T>::begin( ) );
  }
  ...
  {
    this->remove(std::vector<T>::begin() + pos);
  }
  ...
  inline void remove( typename LAArray<T>::iterator it )
  {
    ...
    is_there[key] = false;
    key=-2;
    pool.push_back(it-std::vector<T>::begin());
    len--;
  }
