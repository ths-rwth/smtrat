...
cout << "inform " << _constraint->toString() << endl;
	for( ExpressionConstraintMap::const_iterator iter = expressionOriginMap.begin();
		 iter != expressionOriginMap.end();
		 ++iter )
	{
...
void LRASolverA::printExpressionLAVarMap( ostream& _out ) const
{
	...
	for( ExpressionLAVarMap::const_iterator iter = expressionLAVarMap.begin();
		 iter != expressionLAVarMap.end();
		 ++iter )
	{
...
void LRASolverA::printBacktrackPoints( ostream& _out ) const
{
	...
	for( vector< size_t >::const_iterator iter = backtrack_points.begin();
		 iter != backtrack_points.end();
		 ++iter )
	{
		_out << "                  " << *iter << endl;
	}
...
void LRASolverA::printPushedConstraints( ostream& _out ) const
{
	...
	for( vector< LAVarHistory >::const_iterator iter = pushed_constraints.begin();
		 iter != pushed_constraints.end();
		 ++iter )
	{
...
//			_var->bindRow( _slackVar->basicID(), _pivotValue );
	...
	else
	{
		...
		for( ; it != _var->polynomial.end(); _var->polynomial.getNext( it ) )
		{
			...
			{
				columns[it->key]->binded_rows.add( _slackVar->basicID(), _slackVar->polynomial.add( it->key, columns[it->key]->binded_rows.free_pos(), tmp_r ) );
