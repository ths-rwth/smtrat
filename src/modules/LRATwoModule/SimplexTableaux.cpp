...
namespace smtrat
{
    ...
    pair<bool, numeric> SimplexTableaux::getEntry( int row, int column ) const
    {
        ...
        try
        {    // to find the row, if not exists return false
            rowMap = this->at( row );
    ...
    void SimplexTableaux::setEntry( int row, int column, numeric value )
    {
        map<int, numeric> rowMap;
        try
        {    // to find the row, if not exists create one
            this->at( row );
        }
        catch( ... )
        {
    ...
    map<const Constraint*, const ex> SimplexTableaux::getNonBasicsAsConstraints( BoundMap* boundMap ) const
    {
        map<const Constraint*, const ex>                 setOfConstraints;
        map<const ex, const Constraint*>                 varToConstMap = boundMap->getVarToConstraintMap();
        map<const ex, const Constraint*>::const_iterator itConst;
        map<const ex, int>::const_iterator               itEx;

        for( itEx = this->nonBasicVariablesLocation.begin(); itEx != this->nonBasicVariablesLocation.end(); ++itEx )
        {
            itConst = varToConstMap.find( itEx->first );
            if( itConst != varToConstMap.end() )
            {
                setOfConstraints.insert( pair<const Constraint*, const ex>( itConst->second, itConst->first ) );
            }
            else
            {
                cout << "Could not find corresponding Constraint to ex "
    ...
    void SimplexTableaux::updateRow( int row, int column )
    {
        ...
        for( map<int, numeric>::iterator iter = mapOfRow.begin(); iter != mapOfRow.end(); ++iter )
        {
    ...
    void SimplexTableaux::output_writing( int column, int row, string upd_obj )
    {
        ...
        stream2 << row;
        output += stream2.str();
        cout << upd_obj << output << ") in the current Simplex Tableaux. \n Abort. \n";
    }

    void SimplexTableaux::pivotAndUpdate( const ex basic, const ex nonbasic, const Real* value, BetaMap* assignment )
    {
        ...
        if( rowExisting && columnExisting && basicExisting && nonbasicExisting )
        {
                ...
                if( i->first != row )
                {
                    ...
                    if( currentCoefExisting )
                    {
                        ...
                        for( map<int, numeric>::iterator j = iter_row.begin(); j != iter_row.end(); ++j )
                        {
                            ...
                            {
                                output_writing( j->first, row, upd_obj_pivot );
                            }
                        }
                    }
                    else
                    {
                        output_writing( column, i->first, upd_obj_pivot );
                    }
                }
            }
            ...
            for( map<const ex, int>::iterator var = basicVars.begin(); var != basicVars.end(); ++var )
            {
                ...
                {
                    output_writing( row_num, column, upd_obj_beta );
                }
            }
        }
        else
        {
            output_writing( column, row, upd_obj_pivot );
        }
    }

    bool SimplexTableaux::inform( const Constraint* const constraint, BoundMap* boundMap )
    {
    ...
    void SimplexTableaux::inform( const Constraint* const constraint, GiNaC::symtab variables, vector<ex> coefficients, BoundMap* boundMap )
    {
        ...
        this->basicVariablesLocation.insert( pair<const ex, int>( newVariable, newVariableCount ) );
        for( GiNaC::symtab::const_iterator var = variables.begin(); var != variables.end(); ++var )
        {
            map<const ex, int>::iterator it = this->nonBasicVariablesLocation.find( var->second );
            ...
            if( it == this->nonBasicVariablesLocation.end() )
            {
                ...
                this->nonBasicVariablesLocation.insert( pair<const ex, int>( var->second, columnCount ) );
                columnOfVariable = columnCount;
            }
            else
            {
                // element existing, thus determine the column for the variables
                columnOfVariable = this->nonBasicVariablesLocation.at( var->second );
            }
        ...
        boundMap->addBound( newVariable, constraint, coefficient, constant, constraint->relation() );
    }

    bool SimplexTableaux::assertStrictUpper( const Constraint* const _constraintNew,
                                             const Constraint* const _constraintOld,
                                             Bound* newBound,
                                             Bound* oldBound,
                                             BoundMap* boundMap,
                                             BetaMap* betaMap )
    {
        ...
        newBound->activate();
        oldBound->deactivate();
        map<const Constraint*, const ex>                 nonbasics = this->getNonBasicsAsConstraints( boundMap );
        map<const Constraint*, const ex>::const_iterator itConst   = nonbasics.find( _constraintOld );
    ...
    string SimplexTableaux::toString()
    {
        ...
        for( map<const ex, int>::const_iterator rowVar = this->basicVariablesLocation.begin(); rowVar != this->basicVariablesLocation.end();
                ++rowVar )
        {
            result += "The row of: ";
            ...
            result += sstream.str();
            result += "\n";
            for( map<const ex, int>::const_iterator columnVar = this->nonBasicVariablesLocation.begin();
                    columnVar != this->nonBasicVariablesLocation.end(); ++columnVar )
            {
