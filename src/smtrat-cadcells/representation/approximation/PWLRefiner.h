...
class PWLRefiner {
private:
	inline size_t refine_if_necessary(
        ...
        datastructures::PolyRef poly_ref = ls.get_poly_ref(proj.polys(), ctx, primary_variable, secondary_variable);
        ...
        std::vector<RAN> res_roots;
        if (res_ref.level > under_underlying_sample.size()) {
            res_roots = proj.real_roots(under_underlying_sample, res_ref);
        }

        bool has_intersection = std::any_of(
            res_roots.begin(), res_roots.end(),
            [&ls](const auto& root){
                return root > ls.getDomain().lower() && root < ls.getDomain().upper();
            }
        );

        if (!has_intersection) { return 0; }

        Rational between = midpoint(ls.getDomain().lower(), ls.getDomain().upper());
        ...
        Rational approximated_root = below ? rational_above_experimental<RANApproximation::MIDPOINT>(bound_between)
                                           : rational_above_experimental<RANApproximation::MIDPOINT>(bound_between);

        pwlBuilder.addPoint(between, approximated_root);

        if (depth == 0) { return 1; }

        return refine_if_necessary(
                pwlBuilder,
                segment_index,
                ir,
                sample,
...
public:
	inline datastructures::RootFunction refine(
        ...
        while(segment_index < pwlBuilder.get_number_of_segments()) {
        ...
        return below ? pwlBuilder.buildCompoundMaxMin(ctx, proj.polys(), primary_variable, secondary_variable)
                     : pwlBuilder.buildCompoundMinMax(ctx, proj.polys(), primary_variable, secondary_variable);
    }
};
