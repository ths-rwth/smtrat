...
namespace helper {
     ...
     */
    inline FormulaT to_formula(const carl::vs::case_distinction<Poly>& docc) {
     ...
     */
    static bool addOrMergeTestCandidate(std::vector<carl::vs::substitution<Poly>>& results, const carl::vs::substitution<Poly>& newSubstitution) {
     ...
     */
    static bool generateTestCandidates( std::vector<carl::vs::substitution<Poly>>& results, const carl::Variable& eliminationVar, const FormulaSetT& constraints) {
        SMTRAT_LOG_DEBUG("smtrat.mcsat.vs", "Generating test candidates for " << constraints << " and variable " << eliminationVar);
        
        // add minus infinity
        carl::vs::substitution<Poly> sub (eliminationVar, carl::vs::substitution_term<Poly>::minus_infty());
        ...
        for (const auto& constraint : constraints) {
            ...
            auto subType = weakConstraint ? carl::vs::substitution_type::NORMAL : carl::vs::substitution_type::PLUS_EPSILON;

            // generate Zeros
            bool res = generateZeros(constraint, eliminationVar, [&](SqrtEx&& sqrtExpression, ConstraintsT&& sideConditions) {
                carl::vs::substitution<Poly> sub(eliminationVar, carl::vs::substitution_term<Poly>(subType, sqrtExpression), std::move(sideConditions));
    ...
    inline bool substitute(const FormulaT& constr, const carl::vs::substitution<Poly>& substitution, FormulaT& result) {
        if (constr.getType() == carl::FormulaType::CONSTRAINT) {
            auto subres = carl::vs::substitute(constr.constraint(), substitution);
            if (!subres) {
            ...
            auto subres = carl::vs::substitute(constr.variableComparison(), substitution);
            if (!subres) {
                SMTRAT_LOG_DEBUG("smtrat.mcsat.vs", "Substitution failed");
                return false;
            } else {
                if (std::holds_alternative<carl::vs::case_distinction<Poly>>(*subres)) {
                    result = helper::to_formula(std::get<carl::vs::case_distinction<Poly>>(*subres));
                } else {
