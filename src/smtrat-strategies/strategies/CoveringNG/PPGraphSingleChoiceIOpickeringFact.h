...
struct CoveringNGSettings : CoveringNGSettingsDefault  {
    ...
    struct formula_evaluation {
        ...
        static auto create(cadcells::datastructures::Projections& proj) {
            return Type(std::bind_front(covering_ng::formula::complexity::pickering_fact, proj), 1, covering_ng::formula::complexity::min_tdeg, false, true, false, false);
        }
    };
};
...
class CoveringNG_PPGraphSingleChoiceIOpickeringFact: public Manager {
public:
	CoveringNG_PPGraphSingleChoiceIOpickeringFact() : Manager() {
		setStrategy(
