...
struct OCSettings : smtrat::mcsat::onecell::BaseSettings {
	...
	static constexpr bool enforce_tarski = true;
...
struct SATSettings : smtrat::SATSettingsMCSAT {
	struct MCSATSettings : mcsat::Base {
		SequentialExplanation<fm::Explanation<fm::DefaultSettings>,icp::Explanation,vs::Explanation,onecell::Explanation<OCSettings>>;
	};
	using VarScheduler = VarSchedulerMinisat;
};
} // namespace internal

class Eval_McsatPpCombThom : public Manager {
public:
	Eval_McsatPpCombThom() : Manager() {
		setStrategy(
